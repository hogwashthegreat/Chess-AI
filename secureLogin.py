...
def userpage():
    if not (session.get('troymerritt_username')):
        ...
        if (len(data) == 0):
            ...
            password = request.values.get("pw")
            query = "SELECT password FROM troymerritt_users WHERE username = %s;"
            queryVars = (username, )
            cursor.execute(query, queryVars)
            mysql.connection.commit()
            data=cursor.fetchall()
            hashedPw = data[0]['password']
            if (check_password_hash(hashedPw, password)):
                return render_template('userpage.html.j2', session = session, data1 = data1, data2 = data2, data3 = data3, prediction = prediction)
            else:
                error = 1
                return render_template('index.html.j2', error = error)
    else:
